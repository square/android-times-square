...
public class CalendarViewPager extends ViewPager{
  ...
  private final CalendarViewPager.MonthAdapter adapter;
...
//        }
  ...
  private class MonthAdapter extends PagerAdapter  {
	  ...
	  private List<ViewGroup> datalist;
	  public MonthAdapter() {
		  inflater = LayoutInflater.from(getContext());
		  datalist = new ArrayList<ViewGroup>();
	}
	...
	public Object instantiateItem(ViewGroup parent, int position) {
      ...
      if (monthView == null) {
        ...
        datalist.add(monthView);
      }
      monthView.init(months.get(position), cells.get(position), displayOnly);
      ((ViewPager) parent).addView(datalist.get(position), 0);
